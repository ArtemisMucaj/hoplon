...
# below fail closed if a pin points back at an older asset. Pinned to v2.0.1,
# which is v2.0.0 plus a docs-only fix — same binary shape.
#
# Override the pin with:
#   CODESEARCH_VERSION=latest bash scripts/download_codesearch_binary.sh

CODESEARCH_VERSION="${CODESEARCH_VERSION:-v2.0.1}"
